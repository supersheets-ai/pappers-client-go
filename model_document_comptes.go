/*
Pappers API

L'API de Pappers vous permet de récupérer des informations et documents sur les entreprises françaises à partir de leur numéro SIREN ou SIRET.  Vous devez indiquer votre clé d'API dans les requêtes, soit via le header `api-key`, soit (déconseillé) en utilisant le paramètre query `api_token`.  L'URL d'accès à l'API est https://api.pappers.fr/v2/  Lien vers la documentation de la V3 : https://www.pappers.fr/api/documentation/v3  Lien vers la documentation de l'API internationale : https://www.pappers.in/api/documentation  L'historique des modifications (changelog) est accessible à l'url suivante : https://www.pappers.fr/api/changelog 

API version: 2.16.0
Contact: support@pappers.fr
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the DocumentComptes type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DocumentComptes{}

// DocumentComptes struct for DocumentComptes
type DocumentComptes struct {
	Document
	// Date de clôture des comptes, au format AAAA-MM-JJ.
	DateCloture *string `json:"date_cloture,omitempty"`
}

// NewDocumentComptes instantiates a new DocumentComptes object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDocumentComptes() *DocumentComptes {
	this := DocumentComptes{}
	return &this
}

// NewDocumentComptesWithDefaults instantiates a new DocumentComptes object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDocumentComptesWithDefaults() *DocumentComptes {
	this := DocumentComptes{}
	return &this
}

// GetDateCloture returns the DateCloture field value if set, zero value otherwise.
func (o *DocumentComptes) GetDateCloture() string {
	if o == nil || IsNil(o.DateCloture) {
		var ret string
		return ret
	}
	return *o.DateCloture
}

// GetDateClotureOk returns a tuple with the DateCloture field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DocumentComptes) GetDateClotureOk() (*string, bool) {
	if o == nil || IsNil(o.DateCloture) {
		return nil, false
	}
	return o.DateCloture, true
}

// HasDateCloture returns a boolean if a field has been set.
func (o *DocumentComptes) HasDateCloture() bool {
	if o != nil && !IsNil(o.DateCloture) {
		return true
	}

	return false
}

// SetDateCloture gets a reference to the given string and assigns it to the DateCloture field.
func (o *DocumentComptes) SetDateCloture(v string) {
	o.DateCloture = &v
}

func (o DocumentComptes) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DocumentComptes) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	serializedDocument, errDocument := json.Marshal(o.Document)
	if errDocument != nil {
		return map[string]interface{}{}, errDocument
	}
	errDocument = json.Unmarshal([]byte(serializedDocument), &toSerialize)
	if errDocument != nil {
		return map[string]interface{}{}, errDocument
	}
	if !IsNil(o.DateCloture) {
		toSerialize["date_cloture"] = o.DateCloture
	}
	return toSerialize, nil
}

type NullableDocumentComptes struct {
	value *DocumentComptes
	isSet bool
}

func (v NullableDocumentComptes) Get() *DocumentComptes {
	return v.value
}

func (v *NullableDocumentComptes) Set(val *DocumentComptes) {
	v.value = val
	v.isSet = true
}

func (v NullableDocumentComptes) IsSet() bool {
	return v.isSet
}

func (v *NullableDocumentComptes) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDocumentComptes(val *DocumentComptes) *NullableDocumentComptes {
	return &NullableDocumentComptes{value: val, isSet: true}
}

func (v NullableDocumentComptes) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDocumentComptes) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


